...
class AdsUtils:
    ...
    async def link_exchange(self, channel: aio_pika.Channel) -> Tuple[aio_pika.Exchange, aio_pika.Queue]:
        ...
        self.logger.info(f"队列 {self.queue_name} 已绑定到交换机 {self.exchange_name}，路由键 {self.routing_key}")
    ...
    async def handle_message(self, msg: Dict[str, Any]) -> None:
        ...
        if action == "volume_update":
            self.logger.warning(f"音量变更事件: {action}")
            volume = body
            self.logger.info(f"音量变更为: {volume}")
            ## 修改 Ubuntu 音量
    ...
    async def handle_activation(self, advert_site_id: str, cancel_event: asyncio.Event) -> None:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"取消语音播放失败: {e}")
            while True:
                # 检查是否收到取消信号
                ...
                if success:
                    ...
                    self.logger.error(f"激活失败，结果: {result}")
                    # 重试
                
            self.logger.info(f"激活流程完成，结果: {success}")
            return success
        except asyncio.CancelledError:
            self.logger.info("激活流程被取消")
            return False
    ...
    async def _on_message(self, message: aio_pika.IncomingMessage) -> None:
        ...
        try:
            ...
            self.logger.exception("JSON 解析失败，reject 不重回队列。body=%r err=%s", message.body[:256], e)
        ...
        try:
            ...
            # 可能是暂态错误，允许重投
            self.logger.exception("暂态失败，nack 重回队列。msg=%s err=%s", payload, e)
            await message.nack(requeue=True)
            return
        except Exception as e:
            # 业务不可恢复错误：丢弃，避免毒消息阻塞
            self.logger.exception("不可恢复错误，reject 不重回队列。msg=%s err=%s", payload, e)
            await message.reject(requeue=False)
            return
